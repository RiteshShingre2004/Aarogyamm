...
export default function Step1Metrics() {
    ...
    const [subStep, setSubStep] = useState<'name' | 'age' | 'weight' | 'height' | 'results'>('name');
    ...
    const [bmiCategory, setBMICategoryText] = useState('');
    const [bmr, setBMR] = useState(0);
    const [tdee, setTDEE] = useState(0);
    const [saving, setSaving] = useState(false);

    // Final values in metric for storage
    ...
    const calculateAndShow = () => {
        ...
        const hCm = heightUnit === 'cm' ? height : (feet * 30.48) + (inches * 2.54);

        setWeightKg(wKg);
        setHeightCm(hCm);

        const bmiVal = calculateBMI(wKg, hCm);
        const bmrVal = calculateBMR(wKg, hCm, age);
        const tdeeVal = calculateTDEE(bmrVal, 'intermediate'); // use intermediate as baseline for active women

        setBMI(parseFloat(bmiVal.toFixed(1)));
        setBMICategoryText(getBMICategory(bmiVal));
    ...
    const handleContinue = async () => {
        ...
        try {
            // Update Auth Profile
            if (auth.currentUser && userName !== user?.displayName) {
            ...
            navigate('/onboarding/step2', { state: { bmi, bmiCategory, tdee } });
        } catch (e) {
            console.error(e);
        } finally {
            setSaving(false);
    ...
    return (
        <div className="min-h-screen px-5 py-8 max-w-lg mx-auto" style={{ background: '#FFF5F8' }}>
            ...
            <p className="font-accent text-2xl mb-8 flex items-center gap-2" style={{ color: '#D4418E' }}>
                Let's get to know you{userName ? `, ${userName}` : ''}! <Sparkles size={24} />
            </p>

            <AnimatePresence mode="wait">
                {subStep === 'name' && (
                    <motion.div key="name" {...slideIn} className="flex flex-col">
                        <h2 className="font-display text-3xl font-bold mb-2" style={{ color: '#1A0A2E' }}>
                            What should we call you?
                        </h2>
                        <p className="mb-8" style={{ color: '#8B7B8B' }}>We'd love to personalise your experience 🌸</p>
                        ...
                        <div className="bg-white/50 rounded-2xl p-4 mb-8">
                            <input
                                ...
                                className="w-full bg-transparent border-b-2 border-[#D4418E]/30 focus:border-[#D4418E] py-2 text-2xl font-semibold outline-none transition-all"
                                style={{ color: '#1A0A2E' }}
                                autoFocus
                            />
                        </div>
                ...
                {subStep === 'age' && (
                    <motion.div key="age" {...slideIn}>
                        <h2 className="font-display text-3xl font-bold mb-2" style={{ color: '#1A0A2E' }}>
                            How old are you, {userName || 'Beautiful'}?
                        </h2>
                        ...
                        <div className="flex flex-col items-center gap-6">
                            ...
                            <input type="range" min="20" max="50" value={age} onChange={e => setAge(+e.target.value)}
                            ...
                            <p className="font-accent text-lg text-center flex items-center gap-2" style={{ color: '#7B2D8B' }}>
                                {age >= 20 && age <= 25 ? <>Amazing! {age} is the perfect age to start, {userName}! <Dumbbell size={18} /></> :
                                    age <= 35 ? <>Perfect! {age} is a great age to commit! <Sparkles size={18} /></> :
                                        <>Wonderful! It's never too late to feel amazing! <Sparkles size={18} /></>}
                            </p>
                            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.97 }}
                                onClick={() => setSubStep('weight')}
                                className="w-full py-4 rounded-2xl text-white font-semibold text-lg flex items-center justify-center gap-2"
                ...
                {subStep === 'weight' && (
                    <motion.div key="weight" {...slideIn}>
                        ...
                        <p className="mb-6 flex items-center gap-1.5" style={{ color: '#8B7B8B' }}>
                            There's no right or wrong number. This helps us personalise your plan <Heart size={14} fill="rgba(139,123,139,0.3)" />
                        </p>
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setSubStep('name')} className="flex-1 py-3 rounded-2xl border font-medium flex items-center justify-center gap-2"
                                style={{ borderColor: 'rgba(212,65,142,0.3)', color: '#D4418E' }}>
                                <ArrowLeft size={18} /> Back
                            </button>
                            <motion.button whileHover={{ scale: 1.02 }} onClick={() => setSubStep('height')}
                                className="flex-2 py-3 px-8 rounded-2xl text-white font-semibold flex items-center justify-center gap-2"
                ...
                {subStep === 'height' && (
                    <motion.div key="height" {...slideIn}>
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => setSubStep('weight')} className="flex-1 py-3 rounded-2xl border font-medium flex items-center justify-center gap-2"
                                style={{ borderColor: 'rgba(212,65,142,0.3)', color: '#D4418E' }}>
                                <ArrowLeft size={18} /> Back
                            </button>
                            <motion.button whileHover={{ scale: 1.02 }} onClick={calculateAndShow}
                                className="flex-2 py-3 px-8 rounded-2xl text-white font-semibold flex items-center justify-center gap-2"
                                style={{ background: 'linear-gradient(135deg, #D4418E, #FF8C42)', flex: 2 }}>
                                Calculate My BMI <Target size={20} />
                            </motion.button>
                        </div>
                    </motion.div>
                )}
                ...
                {subStep === 'results' && (
                    <motion.div key="results" {...slideIn}>
                        <h2 className="font-display text-3xl font-bold mb-2 text-center flex items-center justify-center gap-2" style={{ color: '#1A0A2E' }}>
                            Your Results <Sparkles size={28} className="text-[#D4418E]" />
                        </h2>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                {[
                                    { label: 'Daily Calories', value: `${tdee} kcal`, icon: Flame },
                                    { label: 'Metabolism (BMR)', value: `${bmr} kcal`, icon: Heart },
                                    { label: 'Current Weight', value: `${weightKg.toFixed(1)} kg`, icon: Scale },
                                    { label: 'Measured Height', value: `${heightCm.toFixed(0)} cm`, icon: Ruler },
                                ].map((stat, i) => (
                                    <motion.div key={i} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                                        transition={{ delay: 0.3 + i * 0.1 }}
                                        className="text-center p-3 rounded-2xl bg-white flex flex-col items-center gap-1">
                                        <stat.icon size={22} className="text-[#D4418E] mb-1" />
                                        <div className="font-bold text-sm" style={{ color: '#1A0A2E' }}>{stat.value}</div>
                                        <div className="text-[10px] text-[#8B7B8B] uppercase font-bold tracking-wider">{stat.label}</div>
                                    </motion.div>
                                ))}
                            </div>
                        </motion.div>
                        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={handleContinue}
                            ...
                            className="w-full py-4 rounded-2xl text-white font-semibold text-lg flex items-center justify-center gap-2"
                            style={{ background: 'linear-gradient(135deg, #D4418E, #FF8C42)' }}>
                            {saving ? 'Saving...' : <>See Your Workout Suggestions <ArrowRight size={20} /></>}
                        </motion.button>
                    </motion.div>
                )}
            </AnimatePresence>
